...
class BasePage(object):
    ...
    def find_elements(self, locator, timeout=60):
        """定位一组元素"""
        loc = (locator["by"], locator["value"])  # 元祖
        elements = WebDriverWait(self.driver, timeout, 1).until(EC.presence_of_all_elements_located(loc))
        return elements
    ...
    # def send_enter(self,value):
    #     self.driver.find_element_by_css_selector(value).send_keys(Keys.ENTER)
    ...
    def is_located(self, locator, timeout=40):
...
if __name__ == '__main__':
    ...
    locator = ("id", "kw")
    b_driver.send_keys(locator, "wenshy")
    locator = ("id", "su")
    b_driver.click(locator)
